from api.models import searchKeyWord
...
class Services:
    ...
    # 获取所有类型
    async def get_keyword_en(self, db):
    ...
    # 获取所有日期
    async def get_keyword_date(self, db):
    ...
    # 获取指定时间用户
    async def get_keyword_current_date(self, db, _d):
        users =  db.query(searchKeyWord.first_name).filter(searchKeyWord.created_at.like(f"{_d}" + "%")).group_by(searchKeyWord.first_name).having(searchKeyWord.first_name != None).all()
        _users = list(set(map(lambda x:x.first_name, users)))
        return _users

    # 获取排名序号
    async def get_keyword_current_number(self, db):
    ...
    # 获取所有关键词统计
    async def get_keyword_count_total(self, db, _ens, _dates):
            results = []
            ...
            for _d in _dates:
                ...
                for _e in _ens:
                    _funcs.append(
                            func.sum(case(whens=[(and_(searchKeyWord.en == _e), 1)],else_=0)).label(_d + '_' + _e),
                        )
    ...
    # 获取当前之前下所有用户统计
    async def get_keyword_count_total_load_childs(self, db, _ens, _users, _d):
        results = []
        _funcs = []
        for _u in _users:
            for _e in _ens:
                _funcs.append(
                        func.sum(case(whens=[(and_(searchKeyWord.first_name == _u, searchKeyWord.en == _e), 1)],else_=0)).label(_u + '_' + _e),
                    )
    ...
    # 获取某一个用户的统计
    async def get_keyword_current_user_count_total(self, db, _ens, _level,  _u,_d):
        results = []
        _funcs = []
        for _l in _level:
            for _e  in _ens:
        ...
        qs = db.query(
            *_funcs,
        ).filter(searchKeyWord.created_at.between(_d + ' 00:00:00', _d + ' 23:59:59')).group_by(searchKeyWord.first_name)
