 ...
 -->
                ...
                <SearchItem label="库存">
                    <el-select v-model="searchForm.type"  placeholder="" clearable filterable>
                        <el-option v-for="(item, index)  in weightypes"
                            :key="item"
                            :label="item"
                            :value="item"
                            >
        ...
        <ListHeader :Interval="true"  layout="refresh,download,condition"  @download="exportToExcel" @condition="conditionToExcel" @refresh="getData">
            ...
            <el-button type="primary" size="small" @click="handleCreate"  v-permission="['createSearchWeight,POST']"><el-icon><Plus /></el-icon> 新增</el-button>
            <el-popconfirm
                ...
                <template #reference>
                    <el-button type="danger" size="small" v-permission="['batchDeleteSearchWeight,POST']" plain><el-icon><Delete /></el-icon> 批量删除</el-button>
                </template>
            </el-popconfirm>

            <el-button type="warning" size="small" v-permission="['handleBatchLockWeight,POST']" @click="handleBatchLock"><el-icon><Lock /></el-icon>批量锁定</el-button>
        ...
        <vxe-table
            ...
            :loading="loading" 
            v-permission="['getListSearchWeight,GET']" 
            ...
            <vxe-column prop="owner" title="库存" align="center" width="200">
                <template  #default="{ row }">
                    <span>{{ row.owner ? row.owner.type : '未知' }}</span>
                    <el-button text size="small" circle></el-button>
                </template>
            </vxe-column>



            <vxe-column prop="owner" title="权重域名" align="center" width="200">
                <template  #default="{ row }">
            ...
            <vxe-column  title="操作" fixed="right" width="110" >
                <template #default="{ row }">
                    <el-button 
                        ...
                        v-permission="['modifySearchWeight,PUT']"
                    /> 
                    ...
                    <el-popconfirm
                        ...
                        >
                        <template #reference v-permission="['deleteSearchWeight,DELETE']">
...
import { 
    ...
    deleteSearchWeight,
    lockSearchWeight,
    batchDeleteSearchWeight,
    getSearchWeightGroupBy,
    batchLockSearchWeight,
    getSearchWeightGroupByNumber,
    getSearchWeightGroupByUser,
    getSearchWeightGroupByEn,
    showSearchWeight,
    searchWeight,
    getTaskProgress
} from "@/api/weightSearch.js"
import ListHeader from '@/components/ListHeader.vue';
import FormDrawer from '@/components/FormDrawer.vue';
import { useInitTable,useInitForm } from '@/composables/useCommon'
import { dateFormat,toast,getNextDate } from '@/utils/tools.js'
...
const {
    ...
    getList: getSearchWeightList,
    delete: deleteSearchWeight,
...
const {
    ...
    getData,
    update: updateSearchWeight,
    create: createSearchWeight,
})
...
const handlerChangeStatus = (status,row) => {
    ...
    lockSearchWeight(row.id, is_contact).then((res)=>{
        if(status){
...
//     loading.value = true
//     // 从后端获取数据
//     getSearchWeightList(searchForm).then(async res => {
...
//             };
//             // let data = JSON.parse(JSON.stringify(this.tableData3));  // 如果直接放置数据不行请加上这句
//             await exportExcel(newDate, fields, getCurrentTime() + "_keyword");
//         }, 1000)
...
const remoteMethod = (query) => {
    if (query) {
        ...
        searchWeight({q: query, s: 1}).then(res=> {
            options.value = res
        }).finally(()=> {
...
const handleEditBefore = (row) => {
    ...
    showSearchWeight(row.id).then(res => {
...
//     let _options = keywords.value.map(idx => ({
//         value: idx.id,
//         label:  idx.name,
//     }))
...
function handleMultiDelete () {
    loading.value = true
    batchDeleteSearchWeight(ids.value)
    .then(res => {
        toast(`批量删除数据成功`)
        getData()
    })
...
onMounted(()=> {
    ...
    setTimeout(() => {
        ...
        getSearchWeightGroupBy().then(res=> {
            ...
            weightypes.value = res.weightypes
...
const handleBatchLock = () => {
    ...
    loading.value = true
    batchLockSearchWeight(ids.value)
    .then(res=> {
        toast('锁定成功')
        ids.value = []
        getData()
